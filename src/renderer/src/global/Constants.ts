...
export const APP_DATA_DB_PATH = (fileName: string) =>
  joinPath(APP_DATA_DB_DIR(), `${fileName}.sqlite`)
export const APP_DATA_STORE_PATH = (storeName: string) =>
  joinPath(APP_DATA_STORE_DIR(), `${storeName}.json`)

export const initPath = async () => {
  // 获取应用数据目录
  appData = await appDataDir()
  const items = [APP_DATA_ASSET_DIR(), APP_DATA_DB_DIR(), APP_DATA_STORE_DIR()]
