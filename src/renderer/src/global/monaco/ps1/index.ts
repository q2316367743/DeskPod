...
export const powerShellLanguageDefinition: monaco.languages.IMonarchLanguage = {
  ...
  tokenizer: {
    root: [
      ...
      [
        /\b(?:begin|break|catch|class|continue|data|default|do|dynamicparam|else|elseif|end|enum|exit|filter|finally|for|foreach|from|function|if|in|param|process|return|switch|throw|trap|try|until|using|var|while)\b/,
        'keyword'
      ],
      [
        /\b(?:-and|-or|-xor|-not|-eq|-ne|-gt|-ge|-lt|-le|-like|-notlike|-match|-notmatch|-contains|-notcontains|-in|-notin|-is|-isnot|-as|-replace|-split|-join|-f|-band|-bor|-bxor|-bnot)\b/,
        'operator'
      ],
...
export function registerPowerShellLanguage() {
  ...
  monaco.languages.registerCompletionItemProvider('powershell', {
    ...
    provideCompletionItems: (model, position) => {
      ...
      const keywordSuggestions: monaco.languages.CompletionItem[] = powerShellKeywords.map(
        (label) => ({
          label,
      ...
      const operatorSuggestions: monaco.languages.CompletionItem[] = powerShellOperators.map(
        (label) => ({
          label,
      ...
      const constantSuggestions: monaco.languages.CompletionItem[] = powerShellConstants.map(
        (label) => ({
          label,
      ...
      const cmdletSuggestions: monaco.languages.CompletionItem[] = powerShellCommonCmdlets.map(
        (label) => ({
          label,
