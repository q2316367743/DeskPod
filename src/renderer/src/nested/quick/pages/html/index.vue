  ...
  <t-layout class="h-full w-full">
    <t-header>
      <t-head-menu value="/html">
        ...
        <template #operations>
          <div class="pl-8px">
            <t-button theme="primary" @click="openInstallQuickApp('html', loadApps)">
              <template #icon>
                <AddIcon />
              </template>
              安装快应用
...
const { apps, loading, upgrading, upgradeDialogVisible, upgradingApp, toast } = toRefs(state)
...
const loadApps = async () => {
  ...
  try {
    ...
    state.apps = res.filter((a) => a.from === 'html')
  } catch (e) {
