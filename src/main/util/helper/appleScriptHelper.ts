 ...
 * AppleScript 辅助工具类
 * 提供常用的 macOS 系统交互功能
 * @see https://github.com/ZToolsCenter/ZTools/blob/main/src/main/utils/appleScriptHelper.ts
 */
class AppleScriptHelper {
  ...
  private async execute(script: string): Promise<string> {
    try {
      ...
      logError('[AppleScript] 执行 AppleScript 失败:', error)
      throw error
    }
  }
  ...
  async getFinderPath(): Promise<string | null> {
    try {
      ...
      logError('[AppleScript] 获取访达路径失败:', error)
      return null
    }
  }
  ...
  async getFrontmostApp(): Promise<{
    ...
    try {
      ...
      logError('[AppleScript] 获取当前激活应用失败:', error)
      return null
    }
  }
  ...
  async getFrontmostAppName(): Promise<string | null> {
    try {
      ...
      logError('[AppleScript] 获取当前激活应用名称失败:', error)
      return null
    }
  }
  ...
  async activateAppByName(appName: string): Promise<boolean> {
    try {
      ...
      logError(`[AppleScript] 激活应用 ${appName} 失败:`, error)
      return false
    }
  }
  ...
  async activateAppByBundleId(bundleId: string): Promise<boolean> {
    try {
      ...
      logError(`[AppleScript] 激活应用 ${bundleId} 失败:`, error)
      return false
    }
  }
  ...
  async activateAppByPath(appPath: string): Promise<boolean> {
    try {
      ...
      logError(`[AppleScript] 激活应用 ${appPath} 失败:`, error)
      return false
    }
  }
  ...
  async openInTerminal(path: string): Promise<boolean> {
    try {
      ...
      logError('[AppleScript] 在终端打开路径失败:', error)
      return false
    }
  }
  ...
  async showNotification(title: string, message: string, subtitle?: string): Promise<boolean> {
    try {
      ...
      logError('[AppleScript] 显示通知失败:', error)
      return false
    }
  }
  ...
  async getRunningApps(): Promise<string[]> {
    try {
      ...
      logError('[AppleScript] 获取运行中应用列表失败:', error)
      return []
    }
  }
  ...
  async isAppRunning(appName: string): Promise<boolean> {
    try {
      ...
      logError(`[AppleScript] 检查应用 ${appName} 运行状态失败:`, error)
      return false
    }
  }
  ...
  async quitApp(appName: string): Promise<boolean> {
    try {
      ...
      logError(`[AppleScript] 退出应用 ${appName} 失败:`, error)
      return false
    }
  }
  ...
  async hideApp(appName: string): Promise<boolean> {
    try {
      ...
      logError(`[AppleScript] 隐藏应用 ${appName} 失败:`, error)
      return false
    }
  }
  ...
  async paste(): Promise<boolean> {
    try {
      ...
      logError('[AppleScript] 执行粘贴操作失败:', error)
      return false
    }
  }
}
